...
//вернуть левый дочерний элемент `A[i]`
    private static int LEFT(int i) {
        return (2 * i + 1);
    }

//вернуть правый дочерний элемент `A[i]`
    private static int RIGHT(int i) {
        return (2 * i + 2);
    }

//замена двух индексов в массиве
...
//рекурсивный алгоритм heapify-down.  
    ...
    // получить левый и правый дочерние элементы узла с индексом `i`
        int left = LEFT(i);
        int right = RIGHT(i);
        int largest = i;
    ...
    //поменять местами с дочерним элементом, имеющим большее значение и вызвать heapify-down для дочернего элемента
        if (largest != i) {
...
//удаление элемента с наивысшим приоритетом (присутствует в корне)
    ...
    //вызвать heapify-down на корневом узле
        heapify(sortArr, 0, size - 1);
        return top;
    }

//выполнить пирамидальную сортировку массива `A` размера `n`
    ...
    //build-heap: вызвать heapify, начиная с последнего внутреннего узла до корневого узла
        int i = (n - 2) / 2;
        while (i >= 0) {
            heapify(sortArr, i--, n);
        }
